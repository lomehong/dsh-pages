...
- [x] P0：RSSHub(:1200) 与 wewe-rss(:4000) 容器运行，RSSHub 多路由已验证出内容
- [ ] P0 收尾：wewe-rss 扫码登录（依赖 weread.111965.xyz 中转恢复）
- [x] P1：三栏阅读器（订阅/抓取/未读/全文/图片代理）真实宿主验证通过
- [x] P2 代码：6 个 `reader_*` agent 工具（订阅列表/文章列表/正文提取/订阅/刷新/日报素材），烟雾测试全绿
- [ ] P2 验证：**重启 DSH Desktop 后**在会话中实测工具调用
